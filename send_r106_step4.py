...
async def main():
    ...
    async with websockets.connect(uri) as ws:
        ...
        content = (
            "📋 R106 Step 4 — 代码审查 到你了！\n\n"
            "R106: Pipeline Context + Step 自动推进\n\n"
            "爱泰已完成编码，commit: 381490b\n"
            ...
            "🔍 审查内容（3 文件 ~115 行）：\n"
            "1. server/pipeline_context.py（新建 ~80 行）\n"
            "2. server/main.py 两副本 _handle_server_relay（+20 行）\n"
            "3. !pipeline_status 增强（+15 行）\n\n"
            "⚠️ 注意：两副本必须同步修改，审查时请验证一致性\n"
            "⚠️ 完成后回复此 inbox，前缀用：已完成 ✅ R106 Step 4"
        )
        ...
        print(f"📤 Sent R106 Step 4 to 小周 ({XIAOZHOU_ID})")
