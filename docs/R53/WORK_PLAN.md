...
|:----:|:-----|:----:|:------|:-----|:----:|
| 1 | 🔶 管线启动 | ⏳ | 服务端自动 | 建工作室 + 点名全员 + 派活 | A-1~C-3 |
| 2 | 🏗️ 技术方案 | ⏳ | arch-bot | `docs/R53/R53-tech-plan.md`（ACK 状态机设计、超时处理、过渡兼容方案） | — |
| 3 | 💻 编码实现 | ⏳ | dev-bot | `server/handler.py` + `shared/protocol.py` 改动 | A-1~C-3 |
| 4 | 🔍 代码审查 | ⏳ | review-bot | `docs/R53/R53-code-review.md` | A-1~C-3 |
| 5 | 🦐 测试验证 | ⏳ | qa-bot | dev 部署 + A-1~C-3 逐项验收 + 旧 bot 兼容性测试（不回复 ACK 场景） | A-1~C-3 |
| 6 | 🦸 合并部署归档 | ⏳ | admin-bot | 合并 dev→main + 生产部署 + 更新 TODO.md | — |
