...
| F-8 | **Web 端每条消息显示两遍** — `loadMessages()` 轮询全量刷新无去重，所有 Tab 均受影响 | 🔴 P1 | **R39** | ❌ 本轮必修 — 编码已预提交 `d1d4bb0` |
| F-9 | **Web 端 Tab 页加载空白（服务器挂了）** — R39 Step 1 创建工作室后发现 Web 端 Tab 页显示「加载中」，服务器服务异常 | 🔴 P0 | **R39** | ⏳ 修复中（admin-bot 正在处理） |
| F-10 | **📊 进度 Tab 空白** — `!task_create` 不产出 📊 前缀进度消息，`renderProgressTab()` 过滤不到数据 | 🟡 P2 | **R39** | ❌ 本轮必修 — 需补进度消息生成入口 |
| F-11 | **Hot Standby 信号死锁** — 各虾等信号→无人发信号→继续等的循环，管线无自动推进机制 | 🟡 P2 | **R39** | ⏳ 本轮若来不及修则排入下轮 |
