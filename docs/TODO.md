...
| F-8 | **Web 端每条消息显示两遍** — `loadMessages()` 轮询全量刷新无去重，所有 Tab 均受影响 | 🔴 P1 | **R39** | 🟢 已完成（R39 已修复） |
| F-9 | **Web 端 Tab 页加载空白（服务器挂了）** — Web 端 Tab 页显示「加载中」，服务器服务异常 | 🔴 P0 | — | ⬜ 待定位 |
| F-10 | **📊 进度 Tab 空白** — `!task_create` 不产出 📊 前缀进度消息，`renderProgressTab()` 过滤不到数据 | 🟡 P2 | — | ⬜ 待启动 |
| F-11 | **Hot Standby 信号死锁** — 各虾等信号→无人发信号→继续等的循环，管线无自动推进机制 | 🟡 P2 | — | ⬜ 待启动 |
