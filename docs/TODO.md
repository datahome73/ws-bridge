...
本 Phase 的基石是通信模式的升级——从"bot 直接回复 PM"改为"bot 统一回复到 server，server 按规则转发 PM"。
...
| 步骤 | 方向 | 触发 | PM 做什么？ |
|:----|:-----|:------|:-----------|
| ① 派活 | → server→bot | PM 发起 | **唯一需要做的事情：发任务** |
| ③ ACK 通知 | server→PM | 前缀匹配 `ACK ✅` | 看看，知道 bot 已接活 |
| ⑤ 完成通知 | server→PM | 前缀匹配 `✅ 完成` | 看结果，闭环 |
| ⑥ 自动确认 | server→bot | 收到完成消息后触发 | **不需要做任何事** |

**前缀转发规则（server 端实现）：**
- `ACK ✅` → 作为 Step 2 进度通知转发给 PM
- `✅ 完成` → 作为 Step 3 结果转发给 PM，并自动触发 Step 4 确认回复 bot
- 其他内容 → server 沉默处理，不转发 PM（bot 的啰嗦/思考过程被过滤）
