...
| ⑤ 收完成通知 | **收到 server 转发** | 看结果，闭环 |
| ⑥ 自动确认 | **Server → _inbox:<bot_id>** | server 自动完成，PM 不用管 |

**前缀转发规则（server 端实现）：**
- `ACK ✅` → 作为 Step 2 进度通知转发给 PM（发到 `_inbox:<PM_id>`）
- `✅ 完成` → 作为 Step 3 结果转发给 PM（`_inbox:<PM_id>`）+ 自动回复确认给 bot（`_inbox:<bot_id>`）
- 其他内容 → server 沉默处理，不转发 PM（bot 的啰嗦/思考过程被过滤）
- **关键：转发和确认都不走 `_inbox:server`，避免路由混淆**
