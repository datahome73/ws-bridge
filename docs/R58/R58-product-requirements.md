...
### 1.4 特殊发现：arch/dev 持续异常

| bot | R57 触发情况 | 备注 |
|:---|:------------|:------|
| 🏗️ arch (小开) | ❌ **每轮需 TG 额外触发** | 可能与其自身 Hermes 网关配置有关 |
| 💻 dev (爱泰) | ❌ **每轮需 TG 额外触发** | 同上 |
| 🔍 review (小周) | ❌ 需触发（略少于 arch/dev） | — |
| 🦐 qa (泰虾) | ❌ 需触发 | 但触发后能顺利推动后续 Step |
| 🦸 admin (小爱) | ❌ 需触发 | 部署环节无备用，卡了 TG 通知 |
| 🧐 PM (小谷) | ✅ 正常响应 | — |

> **项目负责人反馈：** arch 和 dev 几乎每次都需要额外的 TG 触发，可能跟它们自身的 bot 网关配置有关，不完全是 `from_name: "系统"` 的问题。
>
> **影响：** 方向 A 的 `from_name` 改造能解决「系统消息被静默」的问题，但 arch/dev 可能还有更深层的原因。如果方向 A 生效后 arch/dev 仍不响应，需要在 Step 2 技术方案中提出备用方案（如消息格式特殊标记或直接 WS 命令触发）。
