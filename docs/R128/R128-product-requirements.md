...
| # | 问题 | 严重度 | 影响范围 | 修复量 |
|:-:|:-----|:-----:|:---------|:------:|
| **B-1** | Web 端派活消息显示两条（经理+系统） | 🔴 P1 | Web 端消息列表 | 1 行删除 |
| **B-3** | `##status` 缺 `in_progress` 图标，已派活显示 ⬜ | 🟡 P2 | `##status` 命令输出 | 1 行新增 |
| **B-4** | 完成消息格式严格正则，bot 偏差即静默不推进 | 🟡 P2 | 管线自动推进 | ~20 行修改 |
| **B-2** | 离线 bot 派活丢失，重试队列不够可靠 | 🟡 P2 | 离线场景派活 | ~30 行修改 |
...
| # | 事项 | 理由 |
|:-:|:-----|:------|
| ❌ | **不改其他 bug** | 仅修 B-1~B-4，其他问题后续轮次 |
| ❌ | **不改 pipeline_engine.py** | R127 尚未合入 main，修改 main.py 即可 |
| ❌ | **不改 `_try_advance_pipeline` 的业务逻辑** | 只放宽正则，不改推进逻辑 |
...
| 操作 | 文件 | 行数 |
|:-----|:-----|:----:|
| ✅ 修改 | `server/ws_server/main.py` | ~29 行 |
| ❌ 不碰 | 其他文件 | — |
