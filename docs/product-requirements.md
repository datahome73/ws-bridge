...
| # | 行号 | 当前代码 | 严重度 |
|:-:|:----|:---------|:-----:|
| 1 | `__init__.py:96` | `mention_keyword = ... or "小爱"` | 🟡 P2 |
| 2 | `__init__.py:133` | `self._mention_keyword = ... or "小爱"` | 🟡 P2 |
| 3 | `__init__.py:434` | `if "@admin" in content or "@小爱" in content:` | 🟡 P2 |
...
### 需求 A：Gateway plugin 「小爱」脱敏

**目标：** 将 3 处硬编码「小爱」替换为 env var 或配置值。

**改动方案：**

| # | 行号 | 替换为 |
|:-:|:----|:-------|
| 1 | `line 96` | `"小爱"` → `_env("MENTION_KEYWORD") or "admin-bot"` |
| 2 | `line 133` | `"小爱"` → `_env("MENTION_KEYWORD") or self._mention_keyword or "admin-bot"` |
| 3 | `line 434` | `"@小爱"` → 改为 `"@admin"` 判断，或改用 `self._mention_keyword` 变量 |
...
| # | 文件 | 改动内容 |
|:-:|:----|:---------|
| A | `gateway-plugin/__init__.py` | 3 处「小爱」→ env var（~5 行） |
