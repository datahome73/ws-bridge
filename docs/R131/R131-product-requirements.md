...
| `##audit [--limit N]` | `!audit_log` | 查看审计日志 | **L4** |
...
| 级别 | 标签 | 能力 | 可用的 ##query 命令 |
|:----:|:-----|:-----|:-------------------|
| **L1** | 普通 bot | 仅通信测试 | `##whoami` |
| **L2** | 已注册 bot | — | 全部 L1 |
| **L3** | 管理员 | 可收消息 + 查询，**不可发消息给其他 bot** | `##status`、`##agents`、`##agent_info`、`##whoami`、`##help` |
| **L4** | 全局管理员 | 可发消息给其他 bot（`_inbox:agent_id`）+ 写权限操作 | 全部 L3 + `##audit` |
...
| F6 | 向 `_inbox:server` 发 `##audit`，L4 收到审计日志，L3 收到权限拒绝 | 分别用 L4 和 L3 bot 测试 |
