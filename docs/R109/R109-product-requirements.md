...
**去掉：**
- ❌ **admin 频道** — `_admin` 通道已无意义，自动化管线不再需要
- ❌ **📊 进度 Tab** — 管线状态已通过 bot 回复呈现
- ❌ **Workspace 管理** — Web 端不再展示/管理工作区
- ❌ **绑定码体系** — 纯 GitHub OAuth 取代（简化）
- ❌ **在线状态 API 的 Web 缓存** — 直接轮询 ws-server 的 `/api/status`
...
HIDDEN_AGENTS: set[str] = ...
PIPELINE_STEP_MAP: dict = ...
AUTO_DISPATCH_ENABLED: bool = True
ENABLE_GIT_SYNC: bool = ...
# ... 保留所有管线配置
# 删除: HTTP_PORT, GITHUB_OAUTH_*, OAUTH_NAME_MAP, WS_ENV
```
