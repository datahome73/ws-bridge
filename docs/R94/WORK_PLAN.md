---
pipeline:
  name: "R94 — 新 Bot 入驻技能编写 📄"
  work_plan_url: "https://raw.githubusercontent.com/datahome73/ws-bridge/main/docs/R94/WORK_PLAN.md"
  requirements_url: "https://raw.githubusercontent.com/datahome73/ws-bridge/main/docs/R94/R94-product-requirements.md"
  ...
  steps:
    step2:  { role: architect,  title: Gateway 接入指南 }
    step3:  { role: developer,  title: 注册脚本 }
    step4:  { role: reviewer,   title: Inbox 协议精简版 }
    step5:  { role: qa,         title: 排错手册 + 验证清单 }
    step6:  { role: admin,      title: 运维补充 }

  workspace:
    members:
      architect:
        mention_keyword: "architect;架构师"
        rules: "输出 docs/R94/contrib/gateway-config.md，外部视角"
      developer:
        mention_keyword: "developer;开发"
        rules: "输出 docs/R94/contrib/register.py，内联注释完整"
      reviewer:
        mention_keyword: "reviewer;审查"
        rules: "从 docs/inbox-message-protocol.md 提取新 bot 核心内容"
      qa:
        mention_keyword: "qa;测试"
        rules: "输出 docs/R94/contrib/troubleshooting.md + 验证清单"
      admin:
        mention_keyword: "admin;运维"
        rules: "在 gateway-config.md 追加运维部分"
---
...
```
R94 → architect(小开) → developer(爱泰) → reviewer(小周) → qa(泰虾) → admin(小爱) → PM(小谷/汇总)
```
