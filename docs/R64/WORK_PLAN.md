...
| 4 | `@小开` 触发 | 发 `@小开 收到` | 小开回复 |
| 5 | `@arch` 触发 | 发 `@arch 方案` | 小开回复 |
| 6 | `@dev` 触发 | 发 `@dev 编码` | 爱泰回复 |
| 7 | 无触发词静默 | 发普通消息不含任何 keyword | 不回复 |
| 8 | 前缀剥离 | 发 `小开 收到` | 小开收到 `收到` |
| 9 | 前缀剥离 role | 发 `arch 方案` | 小开收到 `方案` |
| 10 | 长词优先 | 触发词含 `小` 和 `小开` | `小开` 优先匹配 |
| 11 | 频道路由 | `@arch` 到工作室 | 路由到 lobby |
| 12 | 环境变量 | `MENTION_KEYWORD="小爱;admin"` | 多值生效 |
