...
| 4 | **数据陈旧** | R82 前的老数据混在 DB 中，R83 开始应清清爽爽 |
...
| 🟢 **R83 起应有干净的起点** | 归档旧数据后，收件箱只显示 R83+ 的消息，界面清晰 |
...
R82 前的旧数据（包含已无意义的 lobby/workspace 频道的消息）应清理：
...
归档后新 DB 只包含 R83 轮次开始后产生的消息，收件箱页面可正常显示。
...
| # | 检查项 | 预期结果 | 测试方法 |
|:-:|:-------|:---------|:---------|
| ✅-24 | `messages.db` 已备份归档 | 旧 DB 已命名为 `.r82-backup` 后缀 | ls data/messages.db* |
| ✅-25 | 新 DB 自动创建 | 服务启动后 messages.db 重新生成（仅含 R83 新消息） | ls data/messages.db |
| ✅-26 | `_archive_state.json` 已清理 | 文件不存在，或已清理 | ls data/_archive_state.json |
...
| **3** | 👨‍💻 Dev | 编码实现（标签改造 + 收件箱修复 + API 清理 + 数据归档脚本） | 30min |
| **4** | 👀 Review | 代码审查 | 15min |
...
| `scripts/archive-r83.sh` | **新增** — 数据归档脚本 | ~10 行 |
| **合计** | | **~135 行净改** |
...
| 版本 | 日期 | 变更 |
|:----:|:-----|:------|
| v1.0 | 2026-07-10 | 初稿 — 前端 Tab 重设计 + 收件箱修复 + 登录清理 + 数据归档 |
