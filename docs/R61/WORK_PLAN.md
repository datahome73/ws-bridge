...
| Step | 角色 | 任务 | 预计工时 |
|:----|:-----|:-----|:--------|
| **A** | PM（需求分析师） | 编写需求文档 | ✅ 已完成 |
| **B** | PM（需求分析师） | 编写 WORK_PLAN | ⬅️ 本文件 |
| **2** | Arch（架构师） | 技术方案——确认 F-19/F-20 代码完整性 | ✅ `be3afd7` |
| **3** | Dev（开发工程师） | 验证执行——启动测试管线，实操验证 | ✅ 实际运行验证 |
|| **4** | Review（审查工程师） | 审查——确认测试结果完整、无遗漏 | ✅ `5aec2bc`（含实际运行验证） |
|| **5** | QA（测试工程师） | 测试报告——输出验收表格 | ⏭️ 跳过（项目负责人拍板） |
|| **6** | PM（需求分析师） | 合并 + 部署 + 经验固化 | ✅ 归档完成 |
...
| # | 检查项 | 方法 | 预期 |
|:-:|:-------|:----|:-----|
| 1 | 创建工作室成员列表 | 观察 `!pipeline_start` 输出 | 显示 bot 名（项目管理、开发工程师等）非 agent ID |
