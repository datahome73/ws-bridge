...
│                                  │                                  │
│① 派活 ─────────────────────────→│────────→ _inbox:<bot_id> ──────→│
│   PM直接发bot收件箱，不走server   │  （普通 inbox 路由，现有逻辑）    │
│                                  │                                  │
│                                  │←── ② ACK ✅ R{轮次} 收到！ ────┤
│                                  │     （发到 _inbox:server）        │
│←── ③ 转发 ACK 进度通知 ──────────┤                                  │
...
| ⑥ 收自动确认 | 从 `_inbox:<bot_id>` 收到 | **不回复**（同现有协议） |
...
| `✅ 完成` | Step 3 完成 | **转发给 PM** + **自动回复确认给 bot** | PM + `_inbox:<bot_id>` |
