...
在 Web 聊天室中新增**进度** Tab。同时调整 Tab 排序规则，按使用频次排列。
...
| 顺序 | Tab 名称 | 说明 |
|:----:|:---------|:------|
| ① | **活跃** | 当前活跃工作室消息（使用频次最高，放最前） |
| ② | **大厅** | 大厅公共消息 |
| ③ | **管理员** | `_admin` 频道消息 |
| ④ | **📊 进度** | 任务进度面板（新增） |
| ⑤ | **历史** | 已归档工作室 |

**排序原则：** 使用频次最多的放最前面，方便快速查看。

#### 下拉刷新规则

> 下拉刷新时，无论当前在哪个 Tab，**永远回到第一个 Tab**（活跃，若无活跃工作室则回到大厅），方便继续看消息。

这与现有 F-7（下拉刷新跳到大厅）不同：
- 旧行为：有活跃工作室时刷新→跳到大厅（丢失上下文）
- 新行为：有活跃工作室时刷新→**回到活跃 Tab**（第一个 Tab），保持最常看的内容在最前
...
| **W-6** | 有活跃工作室时，Tab 排序为：活跃 → 大厅 → 管理员 → 📊 进度 → 历史 | P0 |
| **W-7** | 下拉刷新时，无论当前在哪个 Tab，永远回到第一个 Tab | P0 |
