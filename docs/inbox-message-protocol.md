...
| `from_agent` | 发送者的 agent_id，**回复时用它** |
| `from_name` | 发送者的显示名称 |
...
class MyGatewayPlugin:
    ...
    async def _handle_inbox_message(self, msg: dict):
        ...
        sender_id = msg.get("from_agent") or ""
...
### 8.8 AutoRouter 服务模型（R88 🚂）

R88 引入 **Pipeline AutoRouter** — 独立外挂服务，零 `handler.py` 侵入。

#### 定位

AutoRouter 是一个 **bot 身份的自动化服务**，不是 server 内部模块。它以 WebSocket 连接 ws-bridge，用自己的 api_key 认证，监听 PM 收件箱的转发通知，自动按 WORK_PLAN.md 定义的 `topology.chain` 派活下一棒。
...
```
              ...
              ┌───────────────┼───────────────┐
              │               │               │
           Bot 小爱        Bot 小谷     AutoRouter 🚂
        (ws_e39dc473b110)  (PM)       (独立进程，bot 身份)
```

- AutoRouter 是第 **7 个** bot 身份连接
- 它不会发 READY 信号（不参与 gateway 路由）
- 它只监听 PM 收到的 `✅ 完成` 通知，不接收其他消息
- 零 handler.py 修改：所有逻辑在 `server/auto_router.py`（667 行）

#### 工作方式

| 触发条件 | 自动行为 |
|:---------|:---------|
| PM 收到 bot 的 `✅ 完成` 通知 | AutoRouter 检测完成，根据 `WORK_PLAN.md` 中 `topology.chain` 派活下一棒 |
| 新管线启动（Step 1 完成） | Step 1（PM 需求）完成后的 `✅ 完成` 触发 Step 2 自动派活 |
| 断线重连 | AutoRouter 自带重连逻辑（指数退避），重连后恢复已有管线状态 |
...
python3 -m server.auto_router \\
    --api-key sk_ws_xxx \\
    --pm-agent-id ws_f26e585f6479 \\
    --log-level INFO

# 参数说明
--api-key        AutoRouter 专用的 ws-bridge api_key（在 _api_keys.json 中注册）
--pm-agent-id    PM 的 agent_id（小谷: ws_f26e585f6479）
--ws-url         WebSocket 地址（默认: wss://wsim.datahome73.cloud/ws）
--agent-card-path Agent Card JSON 路径（可选）
```
...
| 项 | 值 |
|:---|:----|
| 服务名 | `auto-router.service`（systemd） |
| 工作目录 | `/opt/ws-bridge` |
| Agent ID | `ws_5d1896c9f170` |
| PM Agent ID | `ws_f26e585f6479`（小谷） |
| 日志 | `/var/log/auto_router.log` |
