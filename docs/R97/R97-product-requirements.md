...
`!pipeline_start` 不是 PM 的"启动动作"，而是**项目负责人的授权按钮**。
...
> 你们要执行命令，会先产生一个授权按钮给我，我点授权按钮一个道理。

PM 写初稿、项目负责人审核、项目负责人按授权按钮（`!pipeline_start`）→ AutoRouter 启动 → 派活 step1 给 PM 做"标注审核通过"的收尾工作。PM 的行为就和其他 bot 完全统一了：**收 inbox → 干活 → 推 git → `✅ 完成`**。
