...
当前：  大厅 | 活跃工作室 | 历史工作室
         │
         ▼
R35：   🛠️ 管理员 | 大厅 | 活跃工作室 | 历史工作室
```

| Tab | 频道 | 说明 | 可见性 |
|:----|:-----|:-----|:-------|
| 🛠️ **管理员** 🆕 | `_admin` | 管理操作界面，发 `!` 命令、看服务端回复 | 仅管理员（登录 Web 时 role=admin 才显示）|
| 🏛️ **大厅** | lobby | 公共消息 | 所有人 |
| 🔧 **活跃工作室** | 当前活跃 workspace | 工作室协作 | 工作室成员 |
| 📦 **历史工作室** | 上一个 workspace | 只读回顾 | 所有人 |

**管理员 Tab 的特殊行为：**
- 仅在 Web 登录用户为 **admin 角色**时显示该 Tab
- 登录用户为普通 member 或未注册时**不显示**（即不暴露管理入口）
- Tab 内显示管理频道的消息流：上行（管理员发的 `!` 命令）+ 下行（服务端回复）
- 样式上可与其他 Tab 有轻微区分（如不同背景色），以示这是「管理控制台」而非聊天室
