"""Handoff step5→step6 for final merge+deploy."""
...
async def main():
    ...
    print('\n=== HANDOFF step5→step6 ===')
    await client.send_message('!step_handoff step5 --output fa975f5')
    ...
    for m in received:
        c = m.get('content', '')
        if any(kw in c for kw in ['step5', 'step6', '当前', '管线状态', '交接', '成员']):
