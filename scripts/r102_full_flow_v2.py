"""R102 全流程测试 v2：不发重试，直接 websocket 原始发送"""
import asyncio
import json
import os
import sys
...
async def main():
    ...
    # 派活 — 直接发原始 payload，不通过 send_message（避免 ACK 重试）
    content = (
        "【R102 全流程测试 v2 🧪】\n\n"
        "小爱好！请完成以下任务：\n\n"
        "1️⃣ 回复 收到 ✅ 确认接活\n"
        "2️⃣ 在 docs/R102/ 目录下创建 R102-e2e-test-report.md，内容包含：\n"
        "   - 测试日期\n"
        "   - 测试环境\n"
        "   - 测试项：派活 → ACK → 完成 全链路\n"
        "   - 测试结果：PASS\n"
